...
// @version      4.444
// @description  Поиск по товарам, мультипоиск по никам продавцов
// @author       vibe.coding
...
(function() {
    'use strict';
// === Banana 🦍🍌 Product Seller Redirect v5 ===
if (
  ...
  function tryRedirectToSellerBanana(attempt = 1) {
    // Универсальный селектор: ищем seller по data-test или просто по href
    let link =
    ...
    if (m && m[1]) {
      // Банановый моментальный переход!
      ...
      if (attempt < 7) {
        ...
        console.log("🦍🍌 [Banana GO] Не удалось найти продавца на странице товара!");
      }
    }
  }
  setTimeout(() => tryRedirectToSellerBanana(), 600);
  return; // Всё, дальше скрипт не выполняется на этой странице
}
...
#vibe-history-list {
    ...
    scrollbar-color: gold #23272b;   /* Цвет полосы — gold, фон — тёмный */
    padding-right: 12px;             /* Сдвигаем текст, чтобы полоса не закрывала */
}
...
/* --- Модальное окно для редактирования закрепа --- */
.vibe-edit-modal-bg {
    position: fixed;
...
`;
    ...
    // Остальные функции пойдут в следующем сообщении...
function getNextCustomBookmarkName() {
    let pins = getPins();
    let base = "Кастомная закладка";
    let nums = pins
        .filter(x=>x.type === "bookmark" && x.desc && x.desc.startsWith(base))
        .map(x=>{
            let m = x.desc.match(/^Кастомная закладка\s*(\d*)/);
...
function goToOrders(orderIds) {
    ...
    setMsg('Открыто заказов: ' + validIds.length, true);
    addToHistory({type:'orders', value: orderIds.join(', '), desc: 'Заказы: ' + validIds.join(', ')});
}
...
function showEditModal(idx, currentDesc) {
    ...
    <form class="vibe-edit-modal">
        <label style="color:gold;font-weight:500;font-size:15px;margin-bottom:3px;">Новое имя закладки/закрепа:</label>
        ...
        <div style="display:flex;gap:8px;justify-content:flex-end;">
            <button type="submit" class="vibe-ok">Сохранить</button>
            <button type="button" class="vibe-cancel">Отмена</button>
        </div>
    </form>
    `;
...
function renderHistory() {
            ...
            ? pins.map((item, i) =>
                `<li class="vibe-history-pin" data-pindex="${i}" title="${item.desc||item.value}">
                    ...
                    <div class="vibe-hist-action">
                        <span class="vibe-edit" title="Переименовать">✏️</span>
                        <span class="vibe-unpin" title="Открепить">📌</span>
                        <span class="vibe-remove" title="Удалить">✖</span>
                    </div>
                </li>`
              ).join('')
            : `<li style="color:#666;font-size:14px;">(Нет закрепов)</li>`;
    }
    ...
    if (titleBar && !document.getElementById('vibe-add-bookmark-btn')) {
        ...
        starBtn.title = 'Добавить кастомную закладку';
        starBtn.textContent = '⭐';
        ...
        hList.innerHTML = hist.length
            ? hist.map((item, i) =>
                `<li class="vibe-history-item" data-hindex="${i}" title="${item.desc||item.value}">
                    <span>${item.desc||item.value}</span>
                    <div class="vibe-hist-action">
                        <span class="vibe-pin" title="Закрепить">📌</span>
                        <span class="vibe-remove" title="Удалить">✖</span>
                    </div>
                </li>`
            ).join('')
            : `<li style="color:#666;font-size:14px;">(История пуста)</li>`;
    }
}
...
function createPanel() {
    ...
    panel.innerHTML = `
      <div id="vibe-history-section" class="vibe-history-section hide">
        <div id="vibe-pins-title-bar" class="vibe-history-title">Закреплённые</div>
        <ul id="vibe-history-pins"></ul>
        <div style="display:flex;align-items:center;justify-content:space-between;">
            <div class="vibe-history-title">История</div>
            <button id="vibe-clear-history-btn" class="vibe-clear-history-btn" title="Удалить всю историю">🗑️ Очистить</button>
        </div>
        <ul id="vibe-history-list"></ul>
      </div>
      <div id="vibe-quickpanel-header-row">
        <div id="vibe-quickpanel-title">Banana GO</div>
        <button id="vibe-history-btn" title="История">🦍</button>
      </div>
      <div id="vibe-jump-input-group">
        <input id="vibe-jump-input" type="text" placeholder="Перейти к..." autocomplete="off" />
        <button id="vibe-jump-search-btn" title="Перейти" class="ds-glow-btn">
      ...
      <div id="vibe-quickpanel-close" title="Свернуть">✕</div>
      <div id="vibe-seller-choose-wrap" style="display:none;"></div>
    `;
    document.body.appendChild(panel);
// 🦍🍌 Banana Monkey Patch: заменяем input на textarea
(function monkeyInputToTextarea() {
    ...
    // --- Блокируем прокрутку страницы, если мышка над историей ---
    setTimeout(() => {
        ...
        hist.addEventListener('wheel', function(e) {
            ...
            e.stopPropagation();
            e.preventDefault();
            // Если есть куда скроллить — скроллим только историю
...
function collapsePanel() {
    ...
    btn.title = "Развернуть быстрый переход";
    btn.innerHTML = '🍌';
...
// 🍌 Универсальный банановый сплиттер: разбивает строку на ники по запятым, переносам, ; и табуляции
function splitNicksBanana(str) {
    return str
        .replace(/[\n\r\t;]+/g, ',') // все переносы, табы, ; превращаем в запятую
        .split(',')
        .map(s => s.trim())
        .filter(Boolean);
}

// Исправленная функция jumpAction с универсальным парсингом заказов:
function jumpAction() {
    let input = document.getElementById('vibe-jump-input');
    if (!input) return;
    let value = input.value.trim();
// ========== Блок поиска по ТОВАРАМ (Banana 🦍🍌 через вкладки) ==========
let products = value
    .replace(/[\s,;]+/g, ',')
    .split(',')
    .map(x => x.trim())
...
if (products.length && products.join(',') === value.replace(/[\s,;]+/g, ',')) {
    if (products.length === 1) {
        // Переходим сразу на страницу товара fixed
        window.location.href = 'https://ggsel.net/catalog/product/' + products[0] + '?exec=1';

        return;
    } else {
        // Открываем все товары в новых вкладках fixed
        products.slice().reverse().forEach(productId => {
            window.open('https://ggsel.net/catalog/product/' + productId + '?exec=1', '_blank');

        });
        clearInput();
        log('Открыты страницы товаров: ' + products.join(', '));
        addToHistory({type:'product', value:products.join(', '), desc:'Товар(ы): ' + products.join(', ')});
        return;
    }
}
    ...
    if (/б\s*а\s*н\s*а\s*н\s*а/i.test(value.replace(/\s+/g, ''))) {
        playBananaEasterEgg();
    ...
    if (!value) {
        log("Введите номер заказа или ник продавца");
        clearInput();
        return;
    }
   ...
   // Универсальное извлечение всех заказов: все числа с 9 и более цифрами
// --- Заказ — только 9 цифр подряд, если НЕТ БУКВ в поле ---
if (!/[a-zA-Zа-яА-ЯёЁ]/.test(value)) {
    let orderMatches = value.match(/\d{9}/g);
    if (orderMatches && orderMatches.length) {
        log('Найдены заказы: ' + orderMatches.join(', '));
        addToHistory({type:'orders', value: orderMatches.join(', '), desc: 'Заказы: ' + orderMatches.join(', ')});
    ...
    if (isEmail(value)) {
        log('Переходим к покупателю по email…');
        addToHistory({type:'email', value:value, desc:'Покупатель: ' + value});
        window.location.href = `https://my.digiseller.ru/asp/goods_of_buyer.asp?Email=${encodeURIComponent(value)}`;
        clearInput();
        return;
    }
    if (isIPv4(value)) {
        log('Переходим к покупателю по IP…');
...
// === Несколько продавцов через любые разделители (banana split) ===
if (
    (/[,\n\r;\t]/.test(value)) && /[a-zA-Zа-яА-ЯёЁ]/.test(value)
) {
    let rawNicks = splitNicksBanana(value);
    let cache = sellerCache();

   // Открываем всех продавцов в ОДНОМ порядке (и из кеша, и не из кеша)
rawNicks.slice().reverse().forEach(nick => {
    ...
    if (sellerId) {
        ...
        addToHistory({ type: 'seller', value: nick, sellerId, desc: `Продавец ${nick}` });
    } else {
        addToHistory({ type: 'nick', value: nick, desc: `Продавец ${nick}` });
...
if (ggselProductMatch) {
    ...
    log('Переходим на товар, чтобы вытащить seller id...');
    addToHistory({type:'ggsel', value: value, desc:`GGSEL товар #${prodId}`});
    ...
    if (sellerId) {
        log(`Найден в кеше! Сразу к сообщениям!`);
        addToHistory({type:'seller', value:value, sellerId, desc:`Продавец ${value}`});
    ...
    if (/[a-zA-Zа-яА-ЯёЁ]/.test(value)) {
        ...
        if (pageSellerId) {
            log(`Нашли ID продавца на странице: ${pageSellerId} — кешируем и переходим!`);
            addToHistory({type:'seller', value:value, sellerId:pageSellerId, desc:`Продавец ${value}`});
        ...
        log("Ищем продавца…");
        addToHistory({type:'nick', value:value, desc:`Продавец ${value}`});
...
function showSellerChoose(nicks, ids, query) {
    ...
    wrap.innerHTML = '';
    wrap.style.display = '';
    let msgHtml = `<div style="color:#ffe37e;font-size:15px;font-weight:bold;margin-bottom:5px;">Выберите продавца:</div>`;
    ...
    if (query && nicks.length) {
        msgHtml += `<div style="margin-top:10px;color:#f6aeae;font-size:13px;">Несколько продавцов совпали с "${query}".</div>`;
    }
...
// Сначала пытаемся взять из window.name (если есть)
...
// === ТОЧНОЕ СОВПАДЕНИЕ по одному нику ===
let pending = (pendingSeller || "").trim().toLowerCase();
let exact = sellerBlocks.find(block => block.nick.trim().toLowerCase() === pending);
if (exact) {
    log(`Точное совпадение: ${exact.nick} → ID ${exact.id}`);
    addToHistory({type:'seller', value:exact.nick, sellerId:exact.id, desc:`Продавец ${exact.nick}`});
...
// === Только если в pendingSeller несколько ников ===
if (pendingSeller.includes(',')) {
    const pendingList = pendingSeller.split(',').map(s => s.trim());
    // Для каждого ника ищем только ПОЛНОЕ совпадение
    let matched = [];
    ...
    if (matched.length > 0) {
        matched.forEach(block => {
            log(`Точное совпадение: ${block.nick} → ID ${block.id}`);
            addToHistory({type:'seller', value:block.nick, sellerId:block.id, desc:`Продавец ${block.nick}`});
    ...
    if (sellerBlocks.length === 1) {
        ...
        log(`Нашли только одного продавца (${sellerBlocks[0].nick}), кешируем!`);
        addToHistory({type:'seller', value:pendingSeller, sellerId, desc:`Продавец ${sellerBlocks[0].nick}`});
    ...
    if (input && input.value && /^\d+$/.test(input.value)) {
        log(`Нашли ID продавца: ${input.value} — кешируем и переходим на сообщения...`);
        addToHistory({type:'seller', value:pendingSeller, sellerId:input.value, desc:`Продавец ${pendingSeller}`});
...
// Блокируем прокрутку страницы, когда мышка над историей (только внутри блока)
document.addEventListener('DOMContentLoaded', function() {
    const hist = document.getElementById('vibe-history-list');
    if (!hist) return;
    hist.addEventListener('wheel', function(e) {
        // Проверяем: можно ли прокрутить историю?
        const scrollTop = hist.scrollTop;
        const scrollHeight = hist.scrollHeight;
        const clientHeight = hist.clientHeight;
        const delta = e.deltaY;
        ...
        // Если есть куда скроллить — прокручиваем только историю
        // Если некуда — блокируем прокрутку страницы в любом случае
        if (
            (delta < 0 && !atTop) ||
            (delta > 0 && !atBottom) ||
            atTop || atBottom
    ...
    // === Banana product extraction 🦍🍌 для ggsel.net ===
    ...
    if (url.searchParams.get('banana_product') === '1' && bananaKey && /^\d+$/.test(url.pathname.split('/').pop())) {
        setTimeout(() => {
            ...
            if (m && m[1]) {
                ...
                // Открываем переписку с продавцом (как для "ссылка"):
                window.open('https://my.digiseller.ru/asp/seller_messages.asp?id_s=' + m[1], '_blank');
                // Можно не закрывать таб, если надо отлаживать. Иначе оставь window.close();
                window.close();
            } else {
