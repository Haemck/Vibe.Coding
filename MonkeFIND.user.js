...
// @version      2.8
// @description  Быстрый и глубокий поиск, окно путей с ID, кросс-доменная история и результаты, восстановление после перехода, "Скопировано!", уникальный префикс mnkfnd
...
(function() {
  ...
  // === UI & ЛОГИКА — ИНИЦИАЛИЗАЦИЯ ПОСЛЕ ЗАГРУЗКИ ДАННЫХ ===
  (async function main() {
...
`;
    ...
    collapseBtn.title = "Развернуть быстрый переход";
    ...
    panel.innerHTML = `
      <button class="mnkfnd-close-btn" title="Закрыть" id="mnkfnd-quickpanel-close">×</button>
      <div class="mnkfnd-header-gold">MonkeFIND</div>
      <div class="mnkfnd-search-row">
        <input type="text" class="mnkfnd-search-inp" placeholder="Поиск..." maxlength="60" />
        <button class="mnkfnd-search-btn">▶</button>
        <button class="mnkfnd-history-btn" title="История">📖</button>
      </div>
    ...
    pathsWin.innerHTML = `
      <div class="mnkfnd-path-title">
        ...
        <span class="mnkfnd-paths-actions">
          <button type="button" id="mnkfnd-copy-paths-btn" style="font-size:14.2px;padding:3px 12px;border-radius:6px;border:1.2px solid gold;background:linear-gradient(90deg,#ffd34e 75%,#ffe37e 100%);color:#232323;font-weight:600;cursor:pointer;box-shadow:0 1.5px 7px #ffd34e22;">Скопировать пути</button>
          <button type="button" id="mnkfnd-copy-ids-btn" style="font-size:14.2px;padding:3px 12px;border-radius:6px;border:1.2px solid gold;background:linear-gradient(90deg,#ffd34e 75%,#ffe37e 100%);color:#232323;font-weight:600;cursor:pointer;box-shadow:0 1.5px 7px #ffd34e22;">Скопировать ID</button>
        </span>
      </div>
      <div class="mnkfnd-paths-search-row">
        <input type="text" class="mnkfnd-paths-search-inp" placeholder="Поиск по путям..." maxlength="80" />
      </div>
    ...
    function showPaths(paths, game) {
      pathsWin.querySelector('#mnkfnd-path-game-name').textContent = `Все пути для: "${game}"`;
      currentPaths = paths;
    ...
    function renderPathsList(paths, searchVal) {
      ...
      paths.forEach((obj, i) => {
        ...
        li.querySelector('.mnkfnd-paths-text').ondblclick = function(e) {
          ...
          li.querySelector('.mnkfnd-paths-text').textContent = 'Скопировано!';
      ...
      if (!paths.length) {
        ul.innerHTML = `<li class="mnkfnd-paths-item" style="color:#e57d7d;">Нет путей</li>`;
      }
    }
    ...
    async function renderHistory() {
      ...
      if (!searchHistory.length) {
        histDiv.innerHTML = '<div class="mnkfnd-history-empty">История пуста</div>';
        return;
      }
    ...
    // --- РЕНДЕР ВСЕХ РЕЗУЛЬТАТОВ ---
    function renderAllResults(showFast, showDeep) {
      resultsDiv.innerHTML = '';
      let q = input.value.trim();
      // Сохраняем актуальные результаты, чтобы не терялись после кликов и смены вкладок
      ...
      function addSection(title, arr, type) {
        ...
        arr.forEach(obj => {
          ...
          let loader = (loadingRowNum === obj.num) ? '<span style="font-size:19px; color:#ffd34e; margin-left:4px;">…</span>' : '';
          item.innerHTML = `
          ...
          nameEl.onclick = function(e) {
            ...
            if (e.ctrlKey || e.metaKey) {
              ...
              nameEl.textContent = 'Скопировано!';
      ...
      addSection('Быстрый поиск:', fastResults, 'fast');
      addSection('Глубокий поиск:', deepResults, 'deep');
    }
    ...
    async function runSearch(val) {
      ...
      renderAllResults([], []);
      resultsDiv.innerHTML = `
        <div class="mnkfnd-section-title" style="color:#ffe37e;">Быстрый поиск...</div>
        <div class="mnkfnd-section-title" style="color:#ffe37e;">Глубокий поиск...</div>
      `;
      GM_xmlhttpRequest({
        method: "GET",
    ...
    // --- Скролл только внутри результатов ---
